export const findAnswer = (question, profile) => {
  ...
  if (normalized.match(/^(hi|hii|hiii|hello|hey|howdy|sup|yo|good morning|good afternoon|good evening)$/)) {
    return `👋 Hi there! I'm Abhishek's virtual assistant. I can tell you about his skills, projects, achievements, certifications, education, and contact info. What would you like to know?`;
  }

  // Projects — specific project queries
  if (normalized.includes('AgriBidX') || normalized.includes('bidding') || normalized.includes('wellness')) {
    const p = projects.find(proj => proj.titleLower.includes('AgriBidX'));
    return p ? `🧘 **${p.title}** (${p.tech.join(', ')})\n\n${p.description}` : "I couldn't find details about that project.";
  }

  if (normalized.includes('Quick response coordination system') || normalized.includes('javascript') || normalized.includes('Mern') || normalized.includes('path finder') || normalized.includes('shortest pathq  ')) {
    const p = projects.find(proj => proj.titleLower.includes('shortest path'));
    return p ? `🗺️ **${p.title}** (${p.tech.join(', ')})\n\n${p.description}` : "I couldn't find details about that project.";
  }

  if (normalized.includes('LexiVerse hub') || normalized.includes('LexiVerse')) {
    const p = projects.find(proj => proj.titleLower.includes('LexiVerse'));
    return p ? `🤝 **${p.title}** (${p.tech.join(', ')})\n\n${p.description}` : "I couldn't find details about that project.";
  }
  ...
  // Who is Abhishek / about
  if (normalized.includes('who is') || normalized.includes('about abhishek') || normalized.includes('introduce') || normalized.includes('summary') || normalized.includes('background')) {
  ...
  if (normalized.includes('project') || normalized.includes('build') || normalized.includes('built') || normalized.includes('portfolio') || normalized.includes('made') || normalized.includes('created') || normalized.includes('developed')) {
    return `🚀 Abhishek has worked on ${profile.projects.length} key projects:\n\n${profile.projects.map((p, i) => `${i + 1}. **${p.title}** (${p.tech.join(', ')})\n   ${p.description}`).join('\n\n')}`;
  }
  ...
  if (mentionedSkill) {
    ...
    return `✅ Yes! Abhishek knows **${mentionedSkill}**. It's listed under his ${categoryName} skills.\n\nFull ${categoryName} stack: ${category ? category[1].join(', ') : mentionedSkill}`;
  }
  ...
  if (normalized.includes('experience') || normalized.includes('internship') || normalized.includes('intern') || normalized.includes('job') || normalized.includes('work experience')) {
    return `💼 Abhishek is a fresher with no prior work experience, but has built strong practical skills through personal projects including a MERN stack wellness platform, algorithm visualizers, and Quick Response Coordination System. He's actively looking for internship and full-time opportunities!\n\n📬 Reach him at: ${profile.basics.email}`;
  }
  ...
  if (normalized.includes('locat') || normalized.includes('where') || normalized.includes('country') || normalized.includes('city') || normalized.includes('based')) {
    return `📍 Abhishek is based in **${profile.basics.location}** and is open to remote opportunities.`;
  }

  // Fallback
  return `🤔 I'm not sure about that! Try asking me about:\n\n• 💻 Skills & tech stack\n• 🚀 Projects\n• 🎓 Education\n• 🏆 Achievements\n• 📜 Certifications\n• 📬 Contact info\n\nOr reach Abhishek directly at **${profile.basics.email}**!`;
};
